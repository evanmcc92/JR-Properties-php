...
$decryptedUnitID = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256,hash('sha256', $key, true),substr($dataUnitID, mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_CBC)),MCRYPT_MODE_CBC,$ivUnitID),"\0");//script to decrypt
                ...
                echo'<h1>Ticket No. '.$row['TicketID'].'</h1>
    
            <article>
                <p><form action="ticket-delete.php" method="post" id="delete-ticket">
                    <input type="hidden" name="TicketID" value="'.$row['TicketID'].'">
                    <input value="Delete" type="submit" class="button">
                    </form></p>';

                    //if statement for resolved forms
                    if ($row['Resolved'] == "Yes"){
                        echo '
                <p><form action="ticket-update.php" method="post">
                    <input type="hidden" name="TicketID" value="'.$row['TicketID'].'">
